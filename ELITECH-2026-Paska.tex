...
A primary goal of MIRA is to provide a reliable framework for Retrieval-Augmented Generation (RAG). To optimize the retrieval of security playbooks, we implement a hybrid search approach that combines keyword-based full-text search with natural language semantic search. We utilize two primary fusion methods to merge results:

\begin{enumerate}
    \item \textbf{Weighted Scoring:} A configurable approach where the final score is a linear combination of normalized semantic and full-text scores:
    \begin{equation}
        FinalScore = (w \cdot NormSemantic) + ((1 - w) \cdot NormFulltext)
    \end{equation}
    \item \textbf{Reciprocal Rank Fusion (RRF):} A robust method to combine multiple rank lists without requiring score normalization:
    \begin{equation}
        RRFscore(d) = \frac{1}{k + rank_{FT}(d)} + \frac{1}{k + rank_{SEM}(d)}
    \end{equation}
\end{enumerate}

The retrieval performance is evaluated using metrics such as $K-Hit$ rate and Mean Reciprocal Rank (MRR), defined as:
\begin{equation}
    MRR = \frac{1}{|Q|} \sum_{i=1}^{|Q|} \frac{1}{rank_i}
\end{equation}
where $|Q|$ represents the total number of queries and $rank_i$ is the position of the first relevant playbook for the $i$-th query.
